import { Cursor, State, store, Token, useStore } from "./store";
...
let cursor = 0;
let cursorIdx = { i: 0, j: 0 } as Cursor
let nearestMatchIndex = 0
...
export const updatePassage = (
  ...
  logWithTime("updatePassage called", { userWords, partial });
  ...
  // Process matches
  const { matches: newMatches, order: newOrder } = updateMatches(userWords, matches, order, partial)
  ...
  logWithTime("updatePassage finished", { newPassage });
...
const updateMatches = (
  ...
  logWithTime("updateMatches called", { userWords, matches, order });
  ...
  const userWordsSplitters = findMatches(userWords, userWordMatches, store.targetWords, matches, order)
  
  // Split userWords into chunks at each splitter index
  const userWordsChunks = splitUserWords(userWords, userWordsSplitters, partial);
  
  // Match chunks with targetWords
  userWordsChunks.map((userWordsChunk, i) => {
    processChunk(userWordsChunk, store.targetWords, matches, order)
  });
...
const updateCurrentUnrecognized = (matches: State[]) => {
  ...
  store.targetWords.forEach((targetWord, j) => {
    ...
    if (state === "C")  { // Valid correct/matches word, CHANGE: remove 'M'
    ...
    if (store.subRecs.has(targetWord) && j < lastCorrectIdx + 2) { 
      ...
      // console.log("ADD", targetWord, "bc", j, "<", lastCorrectIdx, "+ 2")
    }
  })
  console.log("NEW CUR:", store.currentUnrecognized, store.subRecs)
}
...
const processChunk = (
  ...
  const levenshteins = matches.map((state, j) => { // ISSUE: this checks for all matches in targetWords, not just the current chunk
    if (state) { // Not just "M"
      ...
      if (j + userWordsChunk.length >= targetWords.length)
        window.push("")// = [...window, ""]; // Add end string to better fit chunk to the window
  ...
  nearestMatchIndex = argMin(levenshteins);
  console.log("ML:", nearestMatchIndex, nearestMatchLength)
  ...
  matchChunk(userWordsChunk, matchesSlice, targetWords, order, nearestMatchIndex, windowLength)
  ...
  userWordsChunk.map((userWord, i) => {
    targetWords.slice(nearestMatchIndex + p, nearestMatchIndex + windowLength).some((targetWord, j, targetSlice) => {
      ...
      if (matchesSlice[j - 1] !== "C" && (targetSlice[j - 1].length <= 2)) {//subRecs.some((u) => u.includes(targetSlice[j - 1]))) { 
        matchesSlice[j - 1] = "C"; // Make previous word an implied match
...
const splitUserWords = (
  ...
  if (userWordsChunks.length === 1) {
    if (!partial)
      store.lastUserWordsChunk = userWordsChunks.at(0)?.slice(-4) || [] /*?.slice(-4)*/ // Keep last 4 spoken words in cache
    // store.lastUserWordsChunkLastWordIdx = (userWordsSplitters.at(-1)! + userWordsChunks.at(0)!.length) || 0
    // console.log("L:", useStore.getState().lastUserWordsChunk, useStore.getState().lastUserWordsChunkLastWordIdx)
  ...
  if (!partial && userWordsChunks.length) { // && userWordsChunks.at(-1)
...
const updateCursor = (passage: Token[][]) => {
  let k = 0
  // let found = false
  // let cursorIdx = useStore.getState().cursorIdx
  console.log("N:", cursorIdx)
  passage.some((verse, i) => (
    verse.some((token, j) => {
      if (++k === cursor + 1) { // Add one to offset cursor to after that word
        setCursorIdx({i, j}) // Update position
        return true
      }
    })
  ))}

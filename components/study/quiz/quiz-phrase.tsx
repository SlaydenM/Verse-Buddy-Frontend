...
export function QuizPhrase({ reference }: QuizPhraseProps) {
  ...
  const generateNewPhraseData = () => {
    ...
    const endIndex = (phraseIndex === phrases.length) // If not room for rest of verse
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex gap-2">
          <Button onClick={updatePhraseData} className="py-2 mb-2 border bg-white">New</Button>
          <Button onClick={toggleRest} className="py-2 mb-2 border bg-white">{isRestVisible ? "Hide" : "Reveal"}</Button>
        </div>

...
import { startTranscription, stopTranscription } from "@/public/audio-client"
// import "@/styles/alignment.css"
import React from "react"
import { type Token, useStore, store } from "@/public/store"
import { updatePassage } from "@/public/alignment"
...
export function QuizSpeak({ reference }: QuizSpeakProps) {
  ...
  const capitalizeNames = (str: string): string => {
    ...
    return words
      .map((w, i) => {
        if (i == 0 || names.includes(w)) return capFirst(w)
      })
      .join(" ")
  }
  ...
  const handleClear = () => {
    ...
    const clearedPassage = useStore.getState().passage.map(
      (
        verse: Token[], // Fill passage
      ) =>
        verse.map(
          (token: Token) =>
            ({
  ...
  const handleNext = () => {
    ...
    setOutput(
      <span className="results">
        <span>here are some words that i need to trim up, this is the first test.</span>
      </span>,
    )
  ...
  useEffect(() => {
    ...
    initPassage.forEach((verse) => verse.map((token) => targetWords.push(token.word)))
  ...
  const formatPassage = (passage: Token[][]) => {
    // ISSUE: calls repeatedly regardless of change
          ...
          <span key={`v${i}`} style={startsWithNewline ? { marginLeft: "1rem" } : {}} className="mb-2">
            ...
            {verse.map(({ display: word, separator, state }: Token, j) => (
              <React.Fragment key={j}>
                ...
                {state === "C" ? (
                  <>
                    <span key={"w" + j} className="text-green-600">
                      {word}
                    </span>
                  ...
                  <>
                    <span
                      key={"w" + j}
                      className={`text-gray-800 ${state === "I" ? "shadow-[inset_0_-1px_0_0_red]" : "shadow-[inset_0_-1px_0_0_darkgray]"}`}
                    >
                      {word}
                    </span>
                    <span key={"s" + j} className={"text-gray-600"}>
                      {separator}
                    </span>
                  </>
                )}
                ...
                {passage[0][0].state !== null && i == cursorIdx.i && j == cursorIdx.j && (
                  <ChevronUp
                    ...
                    size={14}
                    color="lightblue"
                    className="absolute inline ml-[-0.63rem] mt-5"
                  />
                )}
      ...
      <CardContent>
        <div>
          <div className="flex gap-2">
            <Button
              className="py-2 border bg-gray-200 dark:bg-white text-gray-900 dark:text-gray-900 hover:bg-gray-300 dark:hover:bg-gray-100"
              onClick={useStore.getState().transcribing ? handleStop : handleStart}
            >
          ...
          >
            <div className="px-3">{transcribing ? <Mic color="#ff5959" /> : <MicOff />}</div>
            {/* {output} */}
            <p
              // ref={divRef}
              id="output-box"
            ...
            <p ref={divRef} className="w-full" contentEditable suppressContentEditableWarning onInput={handleInput}>
              {/* {output} */}
            </p>
          </div>

          {/* Passage */}
          <p>{formatPassage(useStore.getState().passage)}</p>

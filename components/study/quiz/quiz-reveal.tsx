...
function generatePassage(reference: BibleReference): Word[][] {
  return reference.text.map((verse: string, i) => {
    ...
    // Fill words
    const wordsArr = tokens.words.map(
      (word: string, j) =>
        ({
          word,
...
export function QuizReveal({ reference }: QuizRevealProps) {
      ...
      return (
        <span key={i}>
          ...
          <span key={`v${i}`} style={startsWithNewline ? { marginLeft: "1rem" } : {}} className="verse mb-2">
            {/* Number */}
            <span className="verseNum transition-opacity duration-0 ease-[steps(1,end)] opacity-100 px-[0.15rem] text-xs text-gray-400 align-super">
              {i + 1}
            </span>
            ...
            {verse.map(({ word, separator }: Word, j) => (
              <React.Fragment key={j}>
                {/* Word and separator */}
                {word && (
                  <span className={`word inline-block relative z-[1] ${separator === "-" ? "" : "mr-1"}`}>
                    {word}
                    {separator}
                  </span>
                )}
  ...
  useEffect(() => {
    ...
    for (let i = 2; i < words.length; i++) {
      ...
      if (currentTop > firstTop + 2) {
        // allow for subpixel rounding
  ...
  function snapToWord(x: number, y: number) {
    ...
    wordElements
      .filter((word) => Math.abs(word.offsetTop - y) < lineHeightRef.current * 2)
      .forEach((word) => {
  ...
  function updateCover(x: number, y: number) {
    ...
    const nextWord = coverPos.current.word?.nextElementSibling as HTMLSpanElement | null
    if (!nextWord) {
      x += 15
    }
    ...
    const i = br / 2.0,
      ii = (br * Math.sqrt(2)) / 2.0,
      iii = (br * Math.sqrt(3)) / 2.0
    const ip = i - br,
      iip = ii - br,
      iiip = iii - br
  ...
  function nextVerse() {
    ...
    const nextWord =
      nextVerse && currentWord === currentWord.parentElement?.lastElementChild
  ...
  useEffect(() => {
    ...
    padding.current = {
      x: Number.parseFloat(style.paddingLeft) || 12,
      y: Number.parseFloat(style.paddingTop) || 12,
    }
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div ref={containerRef} className="relative select-none">
          <div
            className="relative bg-[#44444f] min-h-[50vh] p-0 m-0"
            style={{
              borderRadius: `${borderRadius}px` /*, background: "repeating-linear-gradient(-32deg, #6b6b6b, #5f5f5fff 25%, #6b6b6b 50%)"*/,
            }}
          >
            <div
              ref={coverRef}
              className="cover inset-0 m-0 p-2 pb-4 ease-out duration-100"
            ...
            <div
              ref={dragHandleRef}
              className={`drag-handle text-white absolute top-0 left-0 z-10 ease-out duration-100 px-1 py-1`}
            >
              {isDraggingRef.current ? <CircleDot /> : <Move />}
            </div>
          </div>
        </div>
      </CardContent>

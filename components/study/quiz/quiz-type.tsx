...
import type { State } from "@/public/store"
...
export function QuizType({ reference }: QuizTypeProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const newPassage = passageRef.current.map(
        (verse) => verse.map((word) => ({ ...word })), // deep copy
      )
              ...
              <React.Fragment key={j}>
                ...
                {state === "C" ? (
                  ...
                  <>
                    <span
                      key={"w" + j}
                      className={`text-transparent ${state === "I" ? "shadow-[inset_0_-1px_0_0_red]" : "shadow-[inset_0_-1px_0_0_darkgray]"}`}
                    >
                      {word}
                    </span>

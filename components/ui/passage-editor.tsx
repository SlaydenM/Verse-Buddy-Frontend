...
import { createEditor, type Descendant, Text, Transforms } from "slate"
...
export default function StudyEditor({ className, onPassageDataChange, initReference }: PassageEditorProps) {
    ...
    value.forEach((block: any) => {
      if (block.type === "heading") {
        ...
        block.children.forEach((child: any) => {
          ...
          const verseNum = matches.length > 0 ? Number.parseInt(matches[matches.length - 1], 10) : prevVerseNum
          prevVerseNum = verseNum // Update prev

          // Find first verse num
          if (!startVerse) {
            startVerse = Number.parseInt(matches.at(0), 10) || 1
          }
          ...
          if (newVerses) {
            ...
            if (!/^[0-9]/.test(child.text[0])) {
              // First verse in block has no verse number
          ...
          if (verseNum !== text.length) console.log("WRONG VERSE NUMBERS")
        })
      }
    })
  ...
  return (
    <Slate editor={editor} initialValue={value} onChange={handleChange} onValueChange={setValue}>
      <Editable
        decorate={decorate}
        renderLeaf={renderLeaf}
        renderElement={renderElement}
        placeholder="Paste here..."
        className={"p-3 sm:p-4 lg:p-5 border border-gray-800 rounded-lg touch-manipulation " + (className ?? "")}
        spellCheck
        autoFocus
      />
    </Slate>
